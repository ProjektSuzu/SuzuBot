...
namespace ProjektRin.Components
{
    public class GroupManager
    {
        ...
        private static readonly CommandManager commandManager = CommandManager.Instance;

        internal List<GroupPreference> groupPreference;

        private void LoadPreferences()
        {
            string? json = File.ReadAllText(rootPath + "/groupPreferences.json");
            groupPreference = JsonConvert.DeserializeObject<List<GroupPreference>>(json) ?? new();
        }

        private void SavePreferences()
        {
            string? json = JsonConvert.SerializeObject(groupPreference);
            File.WriteAllText(rootPath + "/groupPreferences.json", json, Encoding.UTF8);
        }

        internal GroupPreference GetPreference(uint groupUin)
        {
            GroupPreference? preference = groupPreference.Find(x => x.GroupUin == groupUin) ?? null;
            if (preference == null)
            {
        ...
        {
            GroupPreference? preference = GetPreference(groupUin);
        ...
        public bool SetDisabledCommandSet(uint groupUin, bool action, string commandSet)
        {
            ...
            string? packageName = commandManager.CmdSets.FirstOrDefault(x => x.Key.Item1.Name == commandSet).Key.Item1.PackageName;
            if (packageName == null)
            {
                throw new InvalidOperationException($"命令集 \"{commandSet}\" 不存在.");
            }
            ...
            if (action)
            {
                if (!preference.DisabledCommandSets.Remove(packageName))
                {
                    return false;
                }
            }
            ...
            {
                if (preference.DisabledCommandSets.Contains(packageName))
                {
                    return false;
                }
                else
                {
                    preference.DisabledCommandSets.Add(packageName);
                }
            }
            SavePreferences();
            return true;
