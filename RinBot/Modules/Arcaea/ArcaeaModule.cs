...
namespace RinBot.Modules.Arcaea;
...
internal class ArcaeaModule : BaseModule
{
    ...
    public async Task Recent(MessageEventArgs eventArgs, bool neko = false)
    {
        ...
        try
        {
            recent = await _client.User.Info(int.Parse(info.UserCode), 1, ArcaeaUnlimitedAPI.Lib.Models.AuaReplyWith.Recent);
        }
